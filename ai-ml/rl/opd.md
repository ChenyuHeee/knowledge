...
| | 自蒸馏 | OPD |
|------|--------|-----|
| Teacher | 模型自己（前 checkpoint） | **更强的模型** |
| 反馈 | 二元（对/错，留/扔） | **生成式**（Teacher 重写完整答案） |
| 学什么 | 只学好的 | 纠正过程 + 回溯反思 |
| 推理链 | 可能丢掉探索过程 | 保留探索→纠错→回溯 |

**自蒸馏**：自己生成 5 条 → RM/规则二分类 → 保留好的 → SFT。

**OPD**：自己生成 → Teacher 重写一条最优推理链 → SFT。Teacher 不止评判对错，而是把烂的重写成示范。

**在 DeepSeek-R1 中的分工**：自蒸馏（Step 3）快速提纯数据拉高下限，OPD（Step 5）用更强 Teacher 把推理过程的精华传给学生提上限。
